# Vision

Les algorithmes de vision (=analyse d'image) révolutionnent le diagnostique médical, l'agriculture de précision, la reconnaissance faciale, ... Et jusqu'aux champs de bataille en Ukraine et ailleurs.
La somme des applications est vertigineuse et pose de nombreuses questions.

- Quelles applications, quels enjeux, quels risques?
- Quels lien entre la vidéo-surveillance et l'imagerie médicale?
- Quelle régulation? RGPD, AI Act
    - Est il possible de réguler des domaines applicatifs sans réguler les algorithmes eux-mêmes?
    - Une régulation dans un contexte dématerialisé et international est elle possible?


## Un nombre d'applications impressionnant

https://plantnet.org

https://encord.com/blog/computer-vision-role-in-cancer-treatment/

https://www.superannotate.com/blog/computer-vision-in-autonomous-vehicles


## Techniques d'imagerie

Depuis 2010, les techniques ont complètement changé (et même à plusieurs reprises: 2012, 2017, 2023, ...), avec des performances impensables dans différentes applications...

- https://www.pinecone.io/learn/series/image-search/imagenet/
- https://medium.com/neuralmagic/2012-a-breakthrough-year-for-deep-learning-2a31a6796e73
- https://stablediffusionweb.com/fr

L'importance du pré-entrainement et du transfert. Les systèmes modernes sont entrainés sur de très grandes bases de données pour extraire des caractéristiques générales puis raffinés sur des bases de données spécifiques pour des applications particulières.

- https://www.baeldung.com/cs/neural-network-pre-training


La génération d'image est un gouffre en terme de ressources... Mais existe-t-il des applications *utiles*?
Quel est le lien entre la tâche de segmentation et la génération d'image?

- https://www.ibm.com/fr-fr/think/topics/image-segmentation


## Quels verrous pour l'IA embarquée?

Quels sont les enjeux pour *embarquer* les modèles? Est ce que ça se réduit au véhicule autonome? 

Y a-t-il des enjeux en santé? Le véhicule autonome est-il un enjeu de santé?

https://www.technexion.com/resources/embedded-vision-vs-machine-vision-everything-you-need-to-know/

https://medium.com/@jldevtech/mobile-ollama-e52633b4c9c9

Comment embarquer des modèles aussi gourmands en ressources?
Quels sont les risques?


## Le scandale Clearview

A-t-on le droit d'utiliser toutes les données du Web? 
Quels biais sont présents dans les algorithmes?
Pourquoi la video-surveillance intelligente fait elle autant débat?
Quels sont les arguments des promoteurs et des détracteurs?

- https://medium.com/htx-s-s-coe/image-super-resolution-a-comparison-between-interpolation-deep-learning-based-techniques-to-25e7531ab207
- https://www.lemonde.fr/pixels/article/2025/10/29/la-societe-de-reconnaissance-faciale-clearview-ai-visee-par-une-plainte-penale-en-europe_6650145_4408996.html?srsltid=AfmBOoprhsP8R_FJ7gpeGgUaxjFOW-thjt5yJd0C01-9gS1RTcYn091x
- https://www.radiofrance.fr/franceculture/podcasts/un-monde-connecte/clearview-ai-l-entreprise-de-reconnaissance-faciale-qui-condamne-notre-vie-privee-7969521